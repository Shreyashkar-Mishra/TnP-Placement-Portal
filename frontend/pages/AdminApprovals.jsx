...
export const AdminApprovals = () => {
  ...
  if (loading)
    return <div className="p-10 text-center">Loading pending approvals...</div>;

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-2xl font-bold text-gray-900 mb-6 flex items-center">
        <ShieldAlert className="w-6 h-6 mr-2 text-blue-900" /> Admin Access
        Requests
      </h1>

      {error && <div className="text-red-500 mb-4">{error}</div>}
      {successMsg && (
        <div className="mb-4 bg-green-100 text-green-800 p-3 rounded flex items-center">
          <Check className="h-4 w-4 mr-2" /> {successMsg}
        </div>
      )}
        ...
        {pendingAdmins.length === 0 ? (
          ...
          <ul className="divide-y divide-gray-200">
            {pendingAdmins.map((admin) => (
              <li
                key={admin._id}
                className="px-4 py-4 sm:px-6 hover:bg-gray-50 transition selection:bg-blue-100"
              >
