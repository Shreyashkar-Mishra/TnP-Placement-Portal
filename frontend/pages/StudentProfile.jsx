...
export const StudentProfile = () => {
  ...
  useEffect(() => {
    if (user) {
      setFormData({
        ...
        twelfthPercent:
          user.profile?.education?.twelfthPercent?.toString() || "",
        bachelorsPercent:
          user.profile?.education?.bachelorsPercent?.toString() || "",
        mastersPercent:
          user.profile?.education?.mastersPercent?.toString() || "",
        mastersCGPA: user.profile?.education?.mastersCGPA?.toString() || "",
        activeBacklogs:
          user.profile?.education?.activeBacklogs?.toString() || "0",
  ...
  const validate = () => {
    ...
    if (!formData.bachelorsPercent)
      return "Bachelor's Percentage is mandatory.";

    // Master's validation: Required (Percent OR CGPA)
    if (!formData.mastersPercent && !formData.mastersCGPA)
      return "Master's details (Percentage or CGPA) are mandatory.";
    ...
    const profileObj = {
      ...
      education: {
        tenthPercent: formData.tenthPercent
          ? parseFloat(formData.tenthPercent)
          : null,
        twelfthPercent: formData.twelfthPercent
          ? parseFloat(formData.twelfthPercent)
          : null,
        bachelorsPercent: formData.bachelorsPercent
          ? parseFloat(formData.bachelorsPercent)
          : null,
        mastersPercent: formData.mastersPercent
          ? parseFloat(formData.mastersPercent)
          : null,
        mastersCGPA: formData.mastersCGPA
          ? parseFloat(formData.mastersCGPA)
          : null,
          ...
          <div className="bg-white shadow-md rounded-lg overflow-hidden border border-gray-200">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-4">
              ...
              <div className="col-span-1 md:col-span-2">
                ...
                <span className="text-gray-900 capitalize font-semibold text-blue-900">
                  {user?.college || "PCCOE"}
                </span>
              </div>
            </div>
          ...
          <div className="bg-white shadow-md rounded-lg overflow-hidden border border-gray-200">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-y-6 gap-x-4">
              <div>
                ...
                <span className="text-gray-900">
                  {user?.profile?.education?.branch || "N/A"}
                </span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500">
                ...
                <span className="text-gray-900">
                  {user?.profile?.education?.passingYear || "N/A"}
                </span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500">
                ...
                <span className="text-gray-900">
                  {user?.profile?.education?.tenthPercent || "N/A"}%
                </span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500">
                ...
                <span className="text-gray-900">
                  {user?.profile?.education?.twelfthPercent || "N/A"}%
                </span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500">
                ...
                <span className="text-gray-900">
                  {user?.profile?.education?.bachelorsPercent || "N/A"}%
                </span>
              </div>
              <div>
                <span className="block text-sm font-medium text-gray-500">
              ...
              <div>
                ...
                <span className="text-gray-900">
                  {user?.profile?.education?.activeBacklogs || "0"}
                </span>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        {message && (
          <div
            className={`p-4 rounded-md border-l-4 shadow-sm ${message.type === "success" ? "bg-green-50 border-green-500 text-green-700" : "bg-red-50 border-red-500 text-red-700"}`}
          >
            {message.text}
          </div>
        )}
        ...
        <div className="bg-white shadow-md rounded-lg border border-gray-200 overflow-hidden">
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">
                Bachelor's Percentage (Avg){" "}
                <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                name="bachelorsPercent"
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">
                Master's Percentage <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                step="0.01"
                name="mastersPercent"
                value={formData.mastersPercent}
                onChange={handleChange}
                placeholder="Or leave blank if CGPA"
                className="block w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
              />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">
                Master's CGPA <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                step="0.01"
                name="mastersCGPA"
                value={formData.mastersCGPA}
                onChange={handleChange}
