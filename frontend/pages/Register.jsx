...
export const Register = () => {
  ...
  const validateForm = () => {
    if (activeTab === UserRole.Student) {
      if (formData.college === "PCCOE") {
        ...
        if (
          !formData.email.endsWith("@pcu.edu.in") &&
          !formData.email.endsWith("@pcu.ed.in")
        ) {
  ...
  const handleSendOtp = async () => {
    ...
    if (activeTab === UserRole.Student) {
      if (
        formData.college === "PCCOE" &&
        !formData.email.endsWith("@pccoepune.org")
      ) {
        setError("For PCCOE, allowed email domain is @pccoepune.org");
        return;
      }
      if (
        formData.college === "PCU" &&
        !formData.email.endsWith("@pcu.edu.in") &&
        !formData.email.endsWith("@pcu.ed.in")
      ) {
    ...
    try {
      ...
      if (res.success) {
        setOtpSent(true);
        setSuccessMsg(
          `OTP sent to ${formData.email}. It expires in 5 minutes.`,
        );
      } else {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]">
      <motion.div
        ...
        <div className="flex justify-center mb-6">
          <div className="flex items-center space-x-4 bg-white p-3 rounded-xl shadow-lg inline-flex">
            <img src="/pcet-logo.png" alt="PCET" className="h-12 w-auto object-contain" />
            <div className="h-10 border-l border-gray-300"></div>
            <img src="/pccoe-logo-new.png" alt="PCCoE" className="h-14 w-auto object-contain" />
          </div>
        </div>
      ...
      <motion.div
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <span
                        className={`text-sm font-medium ${formData.college === "PCCOE" ? "text-blue-900" : "text-gray-700"}`}
                      >
                        PCCOE
                      </span>
                    </div>
                    <div
                      ...
                      onClick={() =>
                        setFormData({ ...formData, college: "PCU", email: "" })
                      }
                    >
                      <input
                        type="radio"
                      ...
                      <span
                        className={`text-sm font-medium ${formData.college === "PCU" ? "text-blue-900" : "text-gray-700"}`}
                      >
                        PCU
                      </span>
                    </div>
              ...
              <div>
                ...
                {activeTab === UserRole.Student && (
                  <p className="mt-1 text-xs text-gray-500">
                    {formData.college === "PCCOE"
                      ? "Only @pccoepune.org allowed"
                      : "Only @pcu.edu.in allowed"}
                  </p>
                )}
                {successMsg && (
                  <p className="mt-1 text-xs text-green-600 font-medium flex items-center">
                    <Check className="h-3 w-3 mr-1" /> {successMsg}
                  </p>
                )}
        ...
        <div className="text-center mt-4">
          <p className="text-xs text-gray-500">
            &copy; PCCoE MCA Training & Placement Cell
          </p>
        </div>
      </motion.div>
    </div>
