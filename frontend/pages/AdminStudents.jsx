...
const AdminStudents = () => {
  ...
  const handlePromote = async (userId) => {
    if (
      !window.confirm(
        "Are you sure you want to promote this student to Admin? They will be able to post jobs.",
      )
    )
      return;
    ...
    try {
      ...
      if (res.success) {
        ...
        setStudents((prev) =>
          prev.map((s) =>
            s._id === userId ? { ...s, roles: [...s.roles, "admin"] } : s,
          ),
        );
      } else {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            View all registered students and manage their roles.
          </p>
        </div>
                ...
                filteredStudents.map((student) => (
                  <tr key={student._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-sm text-gray-500">
                        {student.profile?.education?.branch || "N/A"}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {student.roles.map((role) => (
                        <span
                          key={role}
                          className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full mr-1 ${role === "admin" ? "bg-purple-100 text-purple-800" : "bg-green-100 text-green-800"}`}
                        >
