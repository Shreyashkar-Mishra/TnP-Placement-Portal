...
export const Jobs = () => {
  ...
  const fetchAppliedStatus = async () => {
    try {
      ...
      if (res.success && res.applications) {
        const ids = new Set(res.applications.map((app) => app.job._id));
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Explore and apply for campus placement drives.
          </p>
        </div>
        <form
      ...
      {message && (
        <div
          className={`mb-6 p-4 rounded-sm border-l-4 ${message.type === "success" ? "bg-green-50 border-green-500 text-green-700" : "bg-red-50 border-red-500 text-red-700"}`}
        >
          {message.text}
        </div>
      )}
          ...
          {jobs.length === 0 && (
            <div className="col-span-full text-center py-12 bg-gray-50 rounded-sm border border-dashed border-gray-300">
              <p className="text-gray-500">
                No active opportunities found at this moment.
              </p>
            </div>
          )}
        </div>
                ...
                <div className="mt-2">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-4 text-sm text-gray-600">
                    ...
                    <div>
                      ...
                      {selectedJob.salary.toLocaleString("en-IN")} LPA
                    </div>
                    <div>
                  ...
                  {selectedJob.educationRequirements && (
                    <div className="mt-6 bg-gray-50 p-4 rounded-md">
                      ...
                      <ul className="list-disc pl-5 text-sm text-gray-600 space-y-1">
                        {selectedJob.educationRequirements.tenthPercent && (
                          <li>
                            10th:{" "}
                            {selectedJob.educationRequirements.tenthPercent}%
                          </li>
                        )}
                        {selectedJob.educationRequirements.twelfthPercent && (
                          <li>
                            12th:{" "}
                            {selectedJob.educationRequirements.twelfthPercent}%
                          </li>
                        )}
                        {selectedJob.educationRequirements.bachelorsPercent && (
                          <li>
                            Bachelor's:{" "}
                            {selectedJob.educationRequirements.bachelorsPercent}
                            %
                        ...
                        {selectedJob.educationRequirements.mastersPercent && (
                          <li>
                            Master's:{" "}
                            {selectedJob.educationRequirements.mastersPercent}%
                          </li>
                  ...
                  {selectedJob.consentFormUrl && (
                    <div className="mt-6">
                      <a
                        href={`/${selectedJob.consentFormUrl}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center text-blue-600 hover:underline"
                      >
                        <CheckCircle className="h-4 w-4 mr-2" /> Download
                ...
                <form onSubmit={handleSubmitApplication}>
                  ...
                  {jobToApply.consentFormUrl && (
                    <div className="mb-4">
                      ...
                      <p className="text-xs text-gray-500 mb-2">
                        Please download the template from job details, sign it,
                        and upload here.
                      </p>
                      <input
                        type="file"
                  ...
                  <div className="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                    <button
                      type="submit"
                      disabled={!resumeFile}
                      className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-900 text-base font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:col-start-2 sm:text-sm disabled:opacity-50"
                    >
                      Submit Application
                    </button>
                    <button
